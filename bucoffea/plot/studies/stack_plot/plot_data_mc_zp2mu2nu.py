...
from distributions_zp2mu2nu import distributions, binnings
from datetime import datetime
from tqdm import tqdm

from bucoffea.plot.plotter_zp2mu2nu import plot_data_mc
...
#            'cr_g_vbf'  : f'EGamma_{year}',
            'cr_2m_zp2mu2nu' : f'SingleMuon_{year}'
        }
...
#            'cr_g_vbf' : re.compile(f'(GJets_DR-0p4.*|VBFGamma.*|QCD_data.*|WJetsToLNu.*HT.*).*{year}'),
            'cr_2m_zp2mu2nu' : re.compile(f'(DYJetsToLL_M-10-MLM.*|TT_pow.*|WWTo2L2Nu-pow.*|ZZ.*).*{year}'),
        }
